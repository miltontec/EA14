...
double GetAgentWeight(int agent_id) {
    ...
    // FIX CRÍTICO: Penalización más suave con floor más alto (0.95^N en lugar de 0.85^N)
    // Esto permite recuperación - racha de -10: 0.95^10 = 0.60 vs 0.85^10 = 0.20
    double streakPenalty = MathPow(0.95, m_agentStats[agent_id].consecutive_losses);
    streakPenalty = MathMax(0.50, streakPenalty);  // Hard floor elevado a 0.50

    // Recuperación lenta: La penalización interna se cura un 5% por trade
    m_agentStats[agent_id].penalty_factor =
        MathMin(1.0, m_agentStats[agent_id].penalty_factor + 0.05);
    ...
    // Limitar peso mínimo para que no muera el agente (siempre 0.2 de oportunidad)
    // Antes: MathMax(0.1, ...); Ahora: MathMax(0.2, ...);
    return MathMax(0.2, finalWeight * 2.0); // Multiplicamos por 2 para normalizar alrededor de 1.0
}
